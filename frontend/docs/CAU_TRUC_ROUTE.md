...
| — Sửa booking | Form chỉnh sửa (đổi ngày, số khách...) | Modal hoặc route con `/bookings/:bookingId/edit`, tuỳ độ phức tạp form. ✅ **Đã có, dạng modal** (`EditBookingModal`, `src/components/bookings/`) mở ngay từ `/bookings`, không phải route con — khớp lựa chọn "modal" trong ô Ghi chú gốc, không dùng `/bookings/:bookingId/edit`. |
