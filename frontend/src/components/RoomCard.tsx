...
import { StarRating, RoomTypeBadge } from './ui'
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {room.amenities.slice(0, 4).map((a) => (
            <AmenityPill key={a}>{a}</AmenityPill>
          ))}
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mb-2">
            <StarRating rating={room.averageRating} />
            <span className="text-xs text-slate-500">
              {room.averageRating.toFixed(1)} ({room.reviewCount}) · Floor {room.floor} · {room.area}m²
            </span>
          </div>
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
          <div>
            <span className="text-xl font-bold text-navy">${room.pricePerNight}</span>
            <span className="text-xs text-slate-400 ml-1">/ night · up to {room.maxGuests} guests</span>
          </div>
          <button
            onClick={(e) => { e.stopPropagation(); onBook() }}
