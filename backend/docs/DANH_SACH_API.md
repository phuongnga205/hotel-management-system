...
| 🆕 Gửi email khi đánh giá bị Admin xoá | Event Listener nội bộ (`EventEmitter2`)         | Event `ReviewDeleted` (emit khi `DELETE /admin/reviews/:id` thành công, payload chỉ gồm `reviewId` + `userId` chủ review)                                                                  | Thông báo cho User bằng **1 subject/body cố định**, không chèn ID review và không có lý do xoá trong email.                                           |
| Gửi báo cáo doanh thu tháng            | Cron job động (`SchedulerRegistry` + `CronJob`) | `REPORT_CRON` (mặc định `55 23 28-31 * *`) theo `REPORT_TIME_ZONE`; service chỉ chạy đúng ngày cuối tháng                                                                                  | Tổng hợp tháng hiện tại và gửi email cho Admin qua transactional outbox. Không liên quan route FE.                                                    |
